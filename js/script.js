...
start = () => {
  alert("Search your city to start");
};
var time = new Date();
if(time.getHours() >= 18 || time.getHours() <= 4){
  document.querySelector(".container").style.background = "linear-gradient(to top, rgba(173, 216, 255, 1) 0%, rgba(30, 144, 255, 1) 50%, rgba(30, 144, 255, 1) 100%";
}
else{
  document.querySelector(".container").style.background = "linear-gradient(167deg, rgba(173, 216, 255, 1) 0%, rgba(0, 138, 255, 1) 38%, rgba(255, 255, 255, 1) 100%)";
}
...
setTime = (elem) => {
  document.getElementById("timezone").innerHTML = "Last Updated time: "+ time.getHours() + ":" + time.getMinutes() + ":" + time.getSeconds();
